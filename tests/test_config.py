...
def test_example_registry_has_only_evidence_verified_cities_enabled() -> None:
    ...
    assert registry.by_key("chicago-daily-high-research-seed").status is VerificationStatus.VERIFIED
    assert registry.by_key("miami-daily-high-research-seed").status is VerificationStatus.VERIFIED
    assert registry.by_key("chongqing-daily-high-research-seed").bucket_width_degrees == 1
    assert registry.by_key("chengdu-daily-high-research-seed").unit == "celsius"
    assert {spec.station_id for spec in registry.specs} == {
        "KLGA",
        "KLAX",
        "KORD",
        "KMIA",
