...
维护窗口默认排除后，已确认的执行事实：真实 NO ask 比 `1−YES last` 平均贵 **1.9¢**；$200 买 NO 完整成交率 **85.6%**，$1000 为 **49.8%**。22 个已结算事件与真实深度归档重叠仍为 0，收益继续是 N/A。
...
2. 继续积累新阈值前向已结算样本；每季独立 30 条的门槛不变
3. 挂单排队、短时撤单与夜间行情裁剪只做测量后再决定

已完成：官方状态组件级 5 分钟订阅；2026-08-26 维护最终窗口核实为 04:00–07:30 UTC，历史全量流 17,873 条、检查点 8,266 条按时间维度默认排除。74 次有精确时间的重连中 66 次落在官方窗口，07:35–07:45 的 8 次无法由公告归因；旧版 28 次只有汇总，虽运行区间与维护重叠 10.7 分钟，但逐次时间已永久丢失，不能强行归因。`data-api/trades` 已接入；链上 SQL 已评估为宏观统计备选，本轮未实现。

### 6.4 T8/T10/T11 为何仍是 N/A

T8 双侧分层、T10 动态加仓、T11 邻桶价差都缺"已结算结果 × 真实深度"重叠样本，按停止条件保持 N/A，**没有用代理数据强行回测**。最新重跑仍是 0 个重叠事件。T6 当前有 25 个未结算前向出局桶，维护剔除后 15 分钟深度覆盖率 44.0%（Wilson 95% 26.7%–62.9%），n<30，统计不可靠，不能视为结论。
