...
生成时间：2026-08-27 13:24:20 +08:00。影子回放订单簿数据截止：2026-08-27 13:22:44 +08:00。
...
- **影子成交仍不是执行授权。** touch、queue-aware、trade-through 是由历史盘口和真实相反方向
  ...
  费率；成交价不是可成交 ask/bid。
- **当前只读影子回放没有可用于执行结论的成交重叠。** 现行 `shadow-spread-v1` 在 45 个
  station/market-day、78,182 个真实深度快照中发现 1,163 个配置价带候选；通过季节、盘口和
  健康闸门后产生 8 个影子订单，但归档 token 与 63,534 笔公开成交的重叠为 **N=0**，因此
  queue-aware/trade-through 成交率、周转和 realized PnL 均为 **N/A**。touch 也只有乐观上界，
  不能替代真实排队成交。详见 [影子策略回放报告](data/shadow_spread_strategy_report.md)。
...
| 只读影子限价/价差回放 | 45 个独立 station/market-day、78,182 个快照、1,163 个价带候选；主 queue-aware 8 个影子订单、0 个成交；与公开成交 token 重叠 **N=0**，所以执行结果、周转和 realized PnL 为 N/A。两类固定触发族中，price-band 为 1,163 个候选，weather-market-lag 为 N=0（归档没有对应天气滞后元数据）。 | 订单簿截止 2026-08-27 13:22:44 +08:00；生成 13:24:20 +08:00；详见影子策略报告 |
