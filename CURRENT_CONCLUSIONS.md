...
生成时间：2026-08-27 12:26:35 +08:00。影子回放订单簿数据截止：2026-08-27 11:59:37 +08:00。
...
- **影子成交仍不是执行授权。** touch、queue-aware、trade-through 是由历史盘口和真实相反方向
  ...
  费率；成交价不是可成交 ask/bid。
- **当前只读影子回放没有可用于执行结论的成交重叠。** 现行 `shadow-spread-v1` 在 42 个
  station/market-day、77,981 个真实深度快照中发现 1,163 个配置价带候选；通过季节、盘口和
...
| 只读影子限价/价差回放 | 42 个独立 station/market-day、77,981 个快照、1,163 个价带候选；主 queue-aware 8 个影子订单、0 个成交；与公开成交 token 重叠 **N=0**，所以执行结果、周转和 realized PnL 为 N/A。 | 订单簿截止 2026-08-27 11:59:37 +08:00；生成 12:26:35 +08:00；详见影子策略报告 |
